(ns clojure-repl-experiments.jmx
...
(def log-config (atom {:log-level {:value "INFO"
                                   :attrs {:writable true}}}))

(comment 
  (jmx/register-mbean
   (jmx/create-bean log-config)
   "juraj:name=logging-bean"
   )

  (jmx/unregister-mbean "juraj:name=logging-bean")

  ;; this does work but is far from testing the real-world functionality
  (def my-bean (jmx/create-bean log-config))
  (.setAttribute my-bean (javax.management.Attribute. "a" 3))
  (.getAttribute my-bean)

  (register-mbean
   (create-bean log-config)
   "juraj:name=logging-bean"
   )
  )
...
(defn- build-attribute-info
  ...
  ([attr-name attr-value]
     (build-attribute-info
      ...
      (name attr-name) true true false))

(ns clojure-experiments.testing.matchers
  (:require [clojure.test :refer [deftest is]]
            ;; https://github.com/nubank/matcher-combinators/#usage
            [matcher-combinators.test] ;; adds support for `match?` and `thrown-match?` in `is` expression
            [matcher-combinators.matchers :as m]))

j;; https://github.com/nubank/matcher-combinators/#clojuretest
...
(deftest test-matching-nested-datastructures
  ...
  (is (match? {:band/members [{:name/first "Alfredo1"}
                              {:name/first "Benedito"}]}
